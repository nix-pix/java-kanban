...
public class KVServer {
    ...
    public KVServer() throws IOException {
        ...
        server.createContext("/register", this::register); //GET
        server.createContext("/save", this::save); //POST
        server.createContext("/load", this::load); //GET
    }

    private void load(HttpExchange h) throws IOException { //метод для получения данных по ключу
        try {
            System.out.println("\n/load");
            if (hasNoAuth(h)) {
            ...
            if ("GET".equals(h.getRequestMethod())) {
                ...
                if (value == null) {
                    System.out.println("Данных по этому ключу не найдено");
                    h.sendResponseHeaders(400, 0);
                    return;
                }
                sendText(h, value);
